package com.game.net.test.server;

import com.game.net.server.NetServer;
import com.game.net.server.ServerType;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.ApplicationContext;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;

@SuppressWarnings("unused")
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(locations = {"classpath:spring.xml"})
public class TestServer {

    @Autowired
    private ApplicationContext context;

    @Test
    public void testWebsocketServer() throws Exception {
        NetServer server = new NetServer(9200, ServerType.TP_WEBSOCKET, context);
        server.launch();
        Thread.sleep(1123036);
    }

    @Test
    public void testTcpServer()  throws Exception {
        NetServer server = new NetServer(9200, ServerType.TP_TCP, context);
        server.launch();
        Thread.sleep(1123036);
    }

    @Test
    public void testUdpServer()  throws Exception {
        NetServer server = new NetServer(9200, ServerType.TP_UDP, context);
        server.launch();
        Thread.sleep(1123036);
    }
}
